...
#ifndef debugon
    #define debugon //special for Kirill
#endif
...
static void stack_dump_d(struct_stack *stk, int line, const char *file, const char *func) {
    ...
    if (stk == NULL) {
        ...
        printf("stk->is_dtor = %d\n", stk->is_dtor);
        printf("stk->is_ctor = %d\n", stk->is_ctor);
        ...
        if (check == -1) {
            ...
            printf("\t stk_c[%d].a = %d\n",              check,  stk_c[check].a);
            printf("\t stk_c[%d].addres_data: %d\n", check,  stk_c[check].addres_data);
            printf("\t stk_c[%d].is_ctor = %d\n",     check,  stk_c[check].is_ctor);
            printf("\t stk_c[%d].is_dtor = %d\n",     check,  stk_c[check].is_dtor);
...
int stack_ok(struct_stack *stk) {
    ...
    if (stk != NULL) {
        ...
        check_error(stk->is_dtor == 1);
        check_error(stk->is_ctor == 0);
...
void stack_push(struct_stack *stk, Elent value) {
    ...
    if (stk->a >= stk->copacity) {
        ...
        Elent *helper = (Elent *) realloc(stk->data, stk->copacity * sizeof(Elent));
...
Elent stack_pop(struct_stack *stk) {
    ...
    if (stk->a < stk->copacity / 2) {
        ...
        stk->data = (Elent *) realloc(stk->data, sizeof(Elent) * stk->copacity);
    }
    ...
    if (stk->a > 0) {
        ...
        printf("You are trying to pop from empty stack!!!\n");
    }
...
int stack_copy(struct_stack *stk, struct_stack *stk_cpy) {
    ...
    return create_check_stack(stk_cpy);
#endif
...
static int create_check_stack(struct_stack *stk) {
    ...
    if (stk == NULL) {
        ...
        Elent *helper = (Elent *) calloc(stk_c[check].copacity, sizeof(Elent));

        if (helper == NULL) {
            return -1;

        } else {
            stk_c[check].data = helper;
        }
